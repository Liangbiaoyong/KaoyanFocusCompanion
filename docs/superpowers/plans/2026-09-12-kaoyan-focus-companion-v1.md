...
> **实现时的两处偏离**（计划里的代码有 bug，实做已修正）：
> 1. `chrome.tabs.create({ url: 'edge://extensions' })` 会被浏览器拦截，按钮点了没反应。改为"复制 `edge://extensions` 到剪贴板"按钮，让用户自己粘贴。
> 2. 权限检查原来查的是**当前活动标签**，但用户此刻正站在引导页上，永远查不到本地 PDF。改为 `chrome.tabs.query({})` 扫描**所有**标签，只要有任何一个 `file://` 地址可读就算已开启。
